...
static inline void DedupeDiscoveredManifests(DiscoveredManifest* arr, size_t size, size_t* new_size,
  ...
  for (size_t i = 0; i < size; i++) {
    for (size_t j = i + 1; j < size; j++) {
      ...
      // Compare structures directly by passing their addresses
      if (CompareDiscoveredManifestPath(&arr[i], &arr[j]) == 0) {
        // Free the duplicate data
        free_data(&arr[j]);

        // Overwrite the duplicate with the last element in the flat array
        arr[j] = arr[size - 1];

        size--;
        j--;  // Recheck this index since a new element was moved here
      }
    }
  }
  *new_size = size;
  ...
  if (stack_size > 0) {
    if (!lua_isnil(L, -1) && !lua_istable(L, -1)) {
      ...
      for (size_t i = 1; i <= len; i++) {
        lua_rawgeti(L, -1, (lua_Integer)i);  // Push manifests[i]
        ...
        if (lua_istable(L, -1)) {
          ...
          for (size_t j = 1; j <= sub_len; j++) {
            lua_rawgeti(L, -1, (lua_Integer)j);  // Push manifests[i][j] (e.g., the path dictionary)

            if (lua_istable(L, -1)) {
              // Extract the dictionary properties generated by your glob loop
              DiscoveredManifest* value = &values[num_values];
              ...
              // Legacy/Fallback check if a sub-array contains raw text elements
              DiscoveredManifest* value = &values[num_values];
            ...
            lua_pop(L, 1);  // Pop manifests[i][j]
          }

          lua_pop(L, 1);  // Pop manifests[i]

        } else if (lua_isstring(L, -1)) {
          // Standard flat dictionary parser (handles direct single elements cleanly)
